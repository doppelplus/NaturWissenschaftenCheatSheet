...
\begin{sectionbox}
	...
	\subsection{Atommodell nach Bohr}
		...
		\begin{itemize}
			\item Hauptquantenzahl $\rightarrow n$
			\item Nebenquantenzahl $\rightarrow l$
			\item Magnetische Quantenzahl $\rightarrow m_l$
		\end{itemize}
		\begin{tabular}{ccccc}
			$\mathbf{n}$ &$\mathbf{l}$ & $\mathbf{m_l}$ & \textbf{Max. $\mathbf{e^- }$} & \textbf{Schale im Bohr Atommodell}\\
			\midrule
			1 & 0&0&2&1s\\
			\midrule
			2&0&0&2&2s\\
			...
			&&2&2&3d\\
		\end{tabular}\\
		Man nennt die Atomzustände mit $l = 0$ auch s-Orbitale, die Zustände mit $l = 1$ p-Orbitale, 
		die Zustände mit $l = 2$ d-Orbitale und die Zustände mit $l = 3$ f-Orbitale.
\end{sectionbox}
			...
			\begin{sectionbox}
				\subsection{Arbeit, Leistung, Wirkungsgrad}
					...
					\begin{tabular}{ll}
						...
						v von $m_1$ danach & $v_1 = u_1 \cdot \frac{m_1-m_2}{m_1+m_2}+ u_2\frac{2m_2}{m_1+m_2} $\\
						v von $m_2$ danach & $v_2 = u_2 \cdot \frac{m_2-m_1}{m_1+m_2}+ u_1\frac{2m_1}{m_1+m_2} $\\						
					\end{tabular}\\
